...
const Catalog: React.FC = () => {
    ...
    return filteredProducts.map((product) => (
      <Card key={product.id} className="mb-4" onClick={() => setSelectedProduct(product)}>
        <Card.Body>
          ...
          <Card.Text className='flex-content'><img alt={product.name} className='imagem-cat' src={window.location.origin + '/imagens/' + product.imagem}></img><span>Preço: ${product.price}</span></Card.Text>
  ...
  return (
    <Container className="catalog mt-4">
      <Row>
        <Col md={8}>
